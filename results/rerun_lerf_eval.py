"""
Fresh re-evaluation of GeoKAN-Gamma + Symbolic Repair on the 4 LERF scenes.

Adapted from .kiro_tmp/eval_lerf_scenes.py (which used the wrong model,
geokan_relation_v4.pt / RBF) — fixed here to use geokan_relation_gamma.pt,
matching what the report and final_results.json consistently describe.
Same GT-derivation (derive_relations, on-the-fly geometric ground truth),
same repair-impact-on-R@K logic, otherwise unchanged.

Usage:
    python results/rerun_lerf_eval.py
"""
import sys
sys.path.insert(0, ".")
...
# Pinned exact object counts (from the first, unconstrained pass: 13/10/10/10).
# Using n_exact removes clustering-boundary non-determinism, matching the same
# deterministic approach already used for the tabletop scenes.
...
model.eval()
print(f"Model loaded (Gamma): {sum(p.numel() for p in model.parameters()):,} params")
...
def compute_f1(predictions, gt_set):
    """Pooled precision/recall/F1 between a predicted triple set and GT (mirrors eval_geokan_tabletop.py)."""
...
print("=" * 70)
print("LERF EVALUATION (FRESH RE-RUN): GeoKAN-Gamma + Symbolic Repair")
print("=" * 70)
...
for scene_name in SCENES:
    ...
    print(f"  Before repair: mR@1={r_b[1]*100:.1f}%  mR@3={r_b[3]*100:.1f}%  mR@5={r_b[5]*100:.1f}%  "
          f"F1={f1_before['f1']:.4f}")
    print(f"  After repair:  mR@1={r_a[1]*100:.1f}%  mR@3={r_a[3]*100:.1f}%  mR@5={r_a[5]*100:.1f}%  "
          f"F1={f1_after['f1']:.4f}")
    ...
    per_scene_results[scene_name] = {
        ...
        "f1_before": f1_before["f1"], "precision_before": f1_before["precision"], "recall_before": f1_before["recall"],
        "f1_after": f1_after["f1"], "precision_after": f1_after["precision"], "recall_after": f1_after["recall"],
    }

print(f"\n{'=' * 70}")
print("AGGREGATE RESULTS (4 LERF scenes) -- FRESH RE-RUN, GeoKAN-Gamma")
print(f"{'=' * 70}")
...
with open("results/lerf_results_fresh.json", "w") as f:
    json.dump({
        ...
        "aggregate": {
            ...
            "micro_f1_before": micro_f1_before, "micro_precision_before": micro_p_before, "micro_recall_before": micro_r_before,
            "micro_f1_after": micro_f1_after, "micro_precision_after": micro_p_after, "micro_recall_after": micro_r_after,
            "total_contradictions": total_contradictions, "total_added": total_added,
        },
    }, f, indent=2)
print("\nSaved -> results/lerf_results_fresh.json")
